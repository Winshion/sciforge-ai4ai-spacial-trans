...
args.mps = torch.backends.mps.is_available() if hasattr(torch.backends, 'mps') else False
...
def main():
    ...
    ari = eva(y, y_pred)
    np.save(args.save_dir + '/ari_' + str(ari) + '.npy',y_pred)
    return ari


def shuffling(x, latent):
    idxs = torch.arange(0, x.shape[0]).to(x.device)
    a = torch.randperm(idxs.size(0)).to(x.device)
